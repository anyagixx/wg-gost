...
if [ -z "$HETZNER_IP" ] || [ -z "$TIMEWEB_IP" ]; then
    ...
    echo ""
    echo "Использование:"
    echo "  curl -sSL ... | bash -s -- \"HETZNER_IP\" \"GOST_PASS\" \"TIMEWEB_IP\" \"WG_HASH\""
    echo ""
    echo "Параметры:"
    ...
    echo "  WG_HASH      — Хэш пароля для админки WireGuard"
    echo ""
    echo "Пример:"
    echo '  bash setup.sh "49.12.123.45" "MyPass123" "5.42.125.102" "\$2b\$12\$abc..."'
    exit 1
fi

if [ -z "$WG_PASS_HASH" ]; then
    echo "⚠️  ВНИМАНИЕ: Хэш пароля WireGuard не указан!"
...
WG_CMD="docker run -d --name wg-easy --restart always \
...
# Добавляем хэш пароля если указан
...
# 6. Создаем скрипт маршрутизации
...
iptables -t mangle -D PREROUTING -s 10.8.0.0/24 -j MARK --set-mark 1 2>/dev/null || true
iptables -t mangle -D PREROUTING -s 10.8.0.0/24 -d 10.8.0.0/24 -j MARK --set-mark 0 2>/dev/null || true
iptables -t mangle -D PREROUTING -s 10.8.0.0/24 -m set --match-set ru_ips dst -j MARK --set-mark 0 2>/dev/null || true
...
# 7. Создаем скрипт обновления базы RIPE
...
# 10. Добавляем задачу в Cron для ежедневного обновления
